...
class DESOM:
    ...
    def initialize(self, ae_act='relu', ae_init='glorot_uniform'):
        ...
        som_layer = SOMLayer(self.map_size, name='SOM')(cluster_input)
        # Create DESOM model

        self.model = Model(inputs=[self.autoencoder.input, self.mlp_model.input], outputs=[self.autoencoder.output, som_layer])
    ...
    def fit(self, X_train, y_train=None,
        ...
        save_interval = X_train[0].shape[0] // batch_size * save_epochs # save every save_epochs epochs
        ...
        for ite in range(iterations):
            ...
            if (index + 1) * batch_size > X_train[0].shape[0]:
                # idx = index * batch_size::
                X_batch = [X_train[0][index * batch_size::], X_train[1][index * batch_size::]]
                XY_bach = X_train[0][index * batch_size::]
                if y_train is not None:
                    y_batch = y_train[index * batch_size::]
                index = 0
            else:
                # idx = index * batch_size:(index + 1) * batch_size
                X_batch = [X_train[0][index * batch_size:(index + 1) * batch_size], X_train[1][index * batch_size:(index + 1) * batch_size]]
                XY_bach = X_train[0][index * batch_size:(index + 1) * batch_size]
                if y_train is not None:
                    y_batch = y_train[index * batch_size:(index + 1) * batch_size]
                index += 1
            if X_val is not None:
                if (index_val + 1) * batch_size > X_val[0].shape[0]:
            ...
            # Train on batch
            loss = self.model.train_on_batch(X_batch, [XY_bach, w_batch])
            ...
            if ite % eval_interval == 0:
                ...
                decoded_prototypes = self.decode(self.prototypes[:,0:10])
                ...
                logdict['Ltop'] = loss[1] - logdict['Lkm']
...
if __name__ == "__main__":
    ...
    # Instantiate model
    desom = DESOM(encoder_dims=[x_data.shape[-1], 500, 500, 2000, 10], map_size=args.map_size)
    ...
    if args.dataset in ['mnist', 'fmnist', 'usps']:
        img_size = int(np.sqrt(X_train[0].shape[1]))
